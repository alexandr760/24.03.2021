...
# Функция для вывода с РАЗНЫМИ категориями
async def get_categories() -> List[Item]:
    return await Item.query.distinct(Item.category_name).gino.all()


# Функция для вывода с РАЗНЫМИ подкатегориями в выбранной категории
...
# Функция для подсчета с выбранными категориями и подкатегориями
async def count_items(category_code, subcategory_code=None):
    # Прописываем условия для вывода (категория равняется выбранной категории)
...
# Функция вывода всех items, которые есть в переданных категории и подкатегории
...
# Функция для получения объекта по его айди
